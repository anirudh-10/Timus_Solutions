...
inline void update( int idx, int l, int r, int p, int val ) {
    ...
    tre[idx] = gcd(tre[idx*2], tre[idx*2 + 1]);
}
...
int main() 
{
    ...
    {
        cin>>sign>>num;
        queries.push_back({sign,num});
        copy.push_back({num,i});
    }
    sort(all(copy));
    int k = 0 ;
    int prev=-1;
